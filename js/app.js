...
// === FIREBASE ===
// Config diambil dari firebase-config.js
// Kalau belum diisi, app tetap jalan tanpa Firebase
const FIREBASE_CONFIG = (window.ZENTRA_FIREBASE_CONFIG?.apiKey && 
  window.ZENTRA_FIREBASE_CONFIG.apiKey !== 'GANTI_API_KEY')
...
async function initFirebase() {
  // Skip kalau config belum diisi
  if (!FIREBASE_CONFIG) {
    console.info('Firebase config belum diisi, pakai mode lokal.');
    loadAnnouncementsFallback();
    return;
  }
  try {
    const { initializeApp } = await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js");
    const { getFirestore, collection, query, where, orderBy, onSnapshot, doc, getDoc } = 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Force hide loading setelah 2 detik - apapun yang terjadi
  const _hideLoading = () => {
  ...
  setTimeout(_hideLoading, 2000);
  // Backup: force remove setelah 4 detik
  setTimeout(() => {
    const el = document.getElementById('loading-overlay');
    if (el) el.remove();
  }, 4000);
...
// Firebase helpers diinit di initFirebase()
